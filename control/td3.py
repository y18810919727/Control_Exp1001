from common.network.network_copy import soft_update
from common.network.network_copy import copy_net

from control.base_ac import *
...
class Td3(ACBase):
    ...
    def network_definition(self):
        self.value_network1 = ValueNetwork(self.num_inputs, self.num_actions, self.hidden_size, device=self.device)
        self.value_network2 = ValueNetwork(self.num_inputs, self.num_actions, self.hidden_size, device=self.device)
        self.policy_network = PolicyNetwork(self.num_inputs, self.num_actions, self.hidden_size, device=self.device)

        self.target_value_network1 = ValueNetwork(self.num_inputs, self.num_actions, self.hidden_size, device=self.device)
        self.target_value_network2 = ValueNetwork(self.num_inputs, self.num_actions, self.hidden_size, device=self.device)
        self.target_policy_network = PolicyNetwork(self.num_inputs, self.num_actions, self.hidden_size, device=self.device)
