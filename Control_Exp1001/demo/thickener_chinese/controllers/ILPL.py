...
from Control_Exp1001.demo.thickener_chinese.common.integral_penalty import cal_penalty_u
...
class ILPL(HDP):
    ...
    def update_critic(self,y, c, ny, nc, y_star, penalty):
    ...
    def find_a_star(self, y, c, nc, y_star):
        u_bounds = self.u_bounds
        S = self.env.penalty_calculator.S

        S = torch.FloatTensor(S)
        u_max = torch.FloatTensor(u_bounds[:, 1])
        u_min = torch.FloatTensor(u_bounds[:, 0])
        u_mid = (u_max+u_min)/2
        U = torch.diag((u_max- u_min)/2)
        action = self.actor_nn(torch.cat([y,y_star,c],dim=1)).detach()
        iter = 0
        while True:
            iter += 1
            action.requires_grad = True
            action.grad = None
            # penalty_u = cal_penalty_u(S, u_max, u_min, action)
            # penalty_u=penalty_u.unsqueeze(0)
            ...
            new_action = U.mm(
                torch.tanh(
                    -0.5*self.gamma*(U.mm(S).inverse().t()).mm(action.grad.t())
                )
            ).t() + u_mid
            new_action = (new_action-u_min)/(u_max-u_min)*2-1
            if torch.dist(action, new_action) <1e-6 or iter > 5:
                break
            action = new_action

        return action

    def update_actor(self,y, c, ny, nc, y_star):
        action = self.actor_nn(torch.cat([y,y_star,c],dim=1))
        a_star = self.find_a_star(y, c, nc, y_star).detach()
        loss = self.actor_criterion(action, a_star)
    ...
    def update_model(self,state, action, penalty, next_state, done):
        ...
        while True:
            ...
            loss = self.update_critic(y, c, ny, nc, y_star, penalty)
            self.update_actor(y, c, ny, nc, y_star)

            if loss < self.critic_nn_error_limit or loop_time>30:
                break

        print('train step:',loop_time)
